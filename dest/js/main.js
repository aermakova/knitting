...
$(function () {
  ...
  $(".aside-right li a").click(function () {
    ...
    if (header != null) {
      // если .header с классом .is-sticky есть
      var headerHeight = header.offsetHeight + 20,
          // получаем высоту хедера +20пикселей
      _coordinats = $(link).offset().top - headerHeight;

      $(".aside-right li a").removeClass("active");
      $(this).addClass("active");
      $("html, body").animate({
        "scrollTop": _coordinats
      }, 1000);
      return false;
    }
    ...
    $(this).addClass("active");
    $("html, body").animate({
      "scrollTop": coordinats
    }, 1000);
    return false;
  });
  $(".testi-body").on("show.bs.collapse", function () {
    $(".event-details").find(".testi-btn").text("Cвернуть");
  });
  $(".testi-body").on("hide.bs.collapse", function () {
  ...
  mySettings.virtualSelect.addClass("active"); // File Upload
  ...
  function ekUpload() {
    ...
    function output(msg) {
      // Response
      var m = document.getElementById("file-messages");
      m.innerHTML = msg;
    }

    function parseFile(file) {
      console.log(file.name);
      output("<strong>".concat(encodeURI(file.name), "</strong>")); // var fileType = file.type;
      ...
      if (isGood) {
        ...
        document.getElementById("file-notimage").classList.add("hidden"); // Thumbnail Preview

        document.getElementById("file-image").classList.remove("hidden");
        document.getElementById("file-image").src = URL.createObjectURL(file);
      } else {
        document.getElementById("file-image").classList.add("hidden");
        document.getElementById("file-notimage").classList.remove("hidden"); // document.getElementById("file-start").classList.remove("hidden");
    ...
    function uploadFile(file) {
      ...
      if (xhr.upload) {
        ...
        if (file.size <= fileSizeLimit * 1024 * 1024) {
          ...
          output("Please upload a smaller file (< ".concat(fileSizeLimit, " MB)."));
        }
      }
  ...
  ekUpload(); // активация плагина анимаций цыфр
  // $(document).ready(() => {
  //
  //     let show = true,
  //         countbox = ".numbers";
